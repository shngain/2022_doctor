...
export class WelcomePage implements AfterViewInit {
  ...
  constructor(private actRoute: ActivatedRoute, public ht: HttpClient, public router: Router, public alertController: AlertController, public nav: NavController) {
    this.http = new HTTP();
  ...
  loaddata() {
    ...
    this.ht.post("https://cureplus.online/APIs/profile.php", formData)
      .pipe(
        finalize(() => {
        })
      )
  ...
  call(referenceId, orderId) {
    ...
    this.ht.post("https://cureplus.online/APIs/booking.php", formData)
      .pipe(
        finalize(() => {
        })
      )
